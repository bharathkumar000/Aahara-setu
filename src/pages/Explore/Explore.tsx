...
export const Explore: React.FC = () => {
  ...
  const handleConfirmClaim = () => {
    if (selectedFoodId) {
      ...
      showToast(`Claim confirmed for ${claimQuantity || 'selected quantity'}! The donor has been notified.`);
    }
  };
  ...
  return (
    <div className="explore-container">
      ...
      <div className="search-filter-bar glass">
        ...
        <Button variant="glass" className="map-btn" onClick={() => showToast('Interactive Map View is loading...')}>
      ...
      {filteredItems.length > 0 ? (
        <div className="food-grid">
          {filteredItems.sort((a, b) => b.urgencyScore - a.urgencyScore).map((item) => (
            <Card key={item.id} className={`food-card hover-lift urgency-border-${item.urgencyLevel}`}>
              ...
              <Button fullWidth className="claim-btn-premium" onClick={() => { setSelectedFoodId(item.id); setClaimQuantity(''); }}>
                Claim Now
              </Button>
            </Card>
          ))}
